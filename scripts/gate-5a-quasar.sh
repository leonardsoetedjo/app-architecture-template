#!/usr/bin/env bash
# Gate 5a: Frontend Config Sanity (Quasar)
# Reads allowed hostname from .env.example, blocks hardcoded values
# Usage: scripts/gate-5a-quasar.sh <path-to-boilerplate-dir>

set -euo pipefail

BOILERPLATE_DIR="${1:-boilerplate/quasar}"
...
if [ ! -f "$ENV_FILE" ]; then
    ...
    echo "   Create it with: VITE_API_BASE_URL=http://localhost:9000"
    exit 1
fi
...
if [ -z "$ENV_URL" ]; then
    ...
    echo "   Add: VITE_API_BASE_URL=http://localhost:9000"
    exit 1
fi
...
# Check 1: Hardcoded base path in vite.config.ts
...
# Check 2: Proxy target must be localhost, 127.0.0.1, or ALLOWED_HOST
