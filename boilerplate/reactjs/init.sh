#!/usr/bin/env bash
# init.sh — Start dev environment for reactjs-frontend
set -e

echo "=== Starting reactjs-frontend dev environment ==="
...
for i in {1..30}; do
    if curl -sf http://localhost:5173 2>/dev/null; then
