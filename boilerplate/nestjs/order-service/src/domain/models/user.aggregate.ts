...
export class User {
    ...
    async authenticate(plaintextPassword: string, passwordHasher: IPasswordHasher): Promise<boolean> {
        if (!this.enabled) {
            throw new AuthenticationException('AUTH_USER_DISABLED', 'User account is disabled');
        }
        return passwordHasher.matches(plaintextPassword, this.password.getHashedValue());
    }
    ...
    hasRole(role: Role): boolean { return this.roles.has(role); }
