export class Password {
    ...
    getHashedValue(): string { return this.hashedValue; }
    equals(other: Password): boolean { return other instanceof Password && this.hashedValue === other.hashedValue; }

    static validatePlaintext(plaintext: string): void {
        if (!plaintext || plaintext.trim() === '') throw new Error('AUTH_PASSWORD_EMPTY');
        ...
        if (!/[!@#$%^&*()\-=_+\[\]{};:'",.<>/?]/.test(plaintext)) throw new Error('AUTH_PASSWORD_NO_SPECIAL');
    }
}
