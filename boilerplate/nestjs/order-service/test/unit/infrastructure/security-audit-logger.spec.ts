import {
  SecurityAuditLogger,
  SecurityEventType,
} from "../../../src/infrastructure/logging/security-audit-logger.service";
...
describe("SecurityAuditLogger", () => {
  ...
  it("should log AUTH_SUCCESS as info", () => {
    ...
    expect(infoSpy).toHaveBeenCalledWith(
      expect.stringContaining(SecurityEventType.AUTH_SUCCESS),
    );
  });
  ...
  it("should log AUTH_FAILURE as warn", () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining(SecurityEventType.AUTH_FAILURE),
    );
  });
  ...
  it("should log RATE_LIMIT_EXCEEDED as warn", () => {
    ...
    expect(warnSpy).toHaveBeenCalledWith(
      expect.stringContaining(SecurityEventType.RATE_LIMIT_EXCEEDED),
    );
  });
});
