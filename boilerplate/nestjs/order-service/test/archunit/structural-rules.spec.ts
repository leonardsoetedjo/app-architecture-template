...
function walk(dir: string, ext: string, callback: (file: string) => void): void {
    ...
    it('all domain models should be immutable (readonly or getter-only)', () => {
      ...
      if (fs.existsSync(modelsDir)) {
        walk(modelsDir, '.ts', (file) => {
          ...
          const hasMutableField = /\b(public|protected)?\s*\w+\s*:\s*\w+\s*;/.test(content)
    ...
    it('all value object files should be named *.value-object.ts', () => {
      ...
      if (fs.existsSync(modelsDir)) {
        walk(modelsDir, '.ts', (file) => {
          ...
          if (basename.includes('.enum.') || basename.includes('.aggregate.')) return;
    ...
    it('domain events should be in past tense', () => {
      ...
      if (fs.existsSync(eventsDir)) {
        for (const entry of fs.readdirSync(eventsDir)) {
          ...
          const pastTenseWords = ['placed', 'confirmed', 'cancelled', 'shipped', 'delivered', 'paid', 'processed'];
          const isPastTense = pastTenseWords.some((w) => eventClass.toLowerCase().includes(w));
