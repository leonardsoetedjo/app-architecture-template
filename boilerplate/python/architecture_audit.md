...
- [ ] **Outbox Relay Active**: A background process reads `outbox_events` and publishes to the broker.
...
- [ ] **JobRepository / State Tracking**: The batch framework persists execution state so restarts resume from the last successful chunk.
- [ ] **Undo Column**: Every table modified by batch jobs has a `last_batch_run_id` column populated by the writer.
- [ ] **Undo Procedure**: A documented SQL command can revert all changes for a given `last_batch_run_id`.
